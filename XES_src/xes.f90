...
!
   ...
   read(10,*)o_x,o_y,o_z           ! atomic unit, as well as r
!  o_x = o_x / 0.5291772d0
!  o_y = o_y / 0.5291772d0
!  o_z = o_z / 0.5291772d0
...
!--read g-vectors
   ...
   OPEN(53,file='eigv.dat',status='old',form='formatted')
...
!  double precision :: 
   ...
   double precision :: bessj, sum_orthonorm

   integer nbnd, nv, npw, i, j, k
   DOUBLE PRECISION, allocatable ::  A(:, :)
   complex*16,  allocatable  :: psi_cp(:), psi_ks(:,:)

   OPEN(54,file='diag_lambda.dat',status='old',form='unformatted')
   OPEN(50,file='cp_wf.dat',status='old',form='unformatted')
   OPEN(30,file='tm_aes.dat',status='unknown',form='formatted')
...
!=======================================================================
   ...
   print *, 'number of quasi-particle states to be calculated ', n_st
   ...
   do j = 1, nbnd
      ...
      do i_st = 1, n_st    
       ...
       !    psi_ks(k,i_st) = psi_ks(k,i_st) +psi_cp(k)*A(j,i_st+nv)
            psi_ks(k,i_st) = psi_ks(k,i_st) +psi_cp(k)*A(j,i_st)
            sum_orthonorm = sum_orthonorm + conjg( psi_ks(k,i_st) )*psi_ks(k,i_st)
         enddo
      enddo
   enddo
   ...
   do i_st = 1, n_st
      ...
      do k = 1, npw
         sum_orthonorm = sum_orthonorm + conjg( psi_ks(k,i_st) )*psi_ks(k,i_st)
      enddo
      write(20,*)'normal test ', i_st, n_g, real(sum_orthonorm)
   enddo

   do i_st = 1, n_st    ! main loop (states) 
...
!--output final results: de v.s. |TM|**2
   write(30,*) eps(i_st), tm2(i_st)
   
   
   print *, '   done ',  i_st
   enddo            ! end main loop (states)

   deallocate(psi_cp, psi_ks, A)
